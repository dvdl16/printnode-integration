			...
			hardware.append(
				{
					...
					"capabilities": json.dumps(OrderedDict(zip(capabilities, printer.capabilities)))
		...
		for action in self.actions:
			...
			if action.print_on_condition and not action.allow_inline_batch:
				try:
					...
					frappe.throw(
						_("The Condition '{0}' on Action in Row # {1} is invalid").format(
							action.print_on_condition, action.idx
						)
					)
